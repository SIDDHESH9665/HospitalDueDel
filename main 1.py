...
from request_mail import EmailSchema, fast_mail, _mail
...
hospital_profiling_handler = HospitalProfilingDataHandler(
    "hospital_profiling_data.json")
...
def get_server_ip():
    try:
        ...
        ip_addresses = [ip[4][0]
                        for ip in interfaces if not ip[4][0].startswith('127.')]
...
try:
    ...
    if not os.path.exists(excel_path):
        ...
        if os.path.exists(alternative_path):
            ...
            raise FileNotFoundError(
                f"Excel file not found at either {excel_path} or {alternative_path}")
    ...
    required_columns = ['PARTNER_ID', 'HOSPITAL',
                        'HOSP_TYPE', 'CITY', 'STATE', 'PIN']
    missing_columns = [
...
async def claims_analysis(partner_id: int):
    try:
        ...
        if 'PARTNER_ID' not in df.columns:
            raise HTTPException(
                status_code=500, detail="PARTNER_ID column not found in dataset")
        ...
        logger.debug(
            f"Found {len(partner_data)} records for partner_id {partner_id}")
...
async def add_hospital(hospital_data: dict):
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=400, detail="Invalid hospital data or hospital already exists")
...
async def update_hospital(hospital_id: int, hospital_data: dict):
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=404, detail="Hospital not found or invalid data")
...
try:
    ...
    frontend_public = os.path.join(
        current_dir, "..", "Frontend", "HospIntel", "public")
    frontend_public = os.path.normpath(frontend_public)

    if os.path.exists(frontend_public):
        app.mount(
            "/img", StaticFiles(directory=os.path.join(frontend_public, "img")), name="img")
...
async def serve_app(full_path: str):
    ...
    try:
        ...
        index_path = os.path.join(os.path.dirname(os.path.abspath(
            __file__)), "..", "Frontend", "HospIntel", "dist", "index.html")
...
@app.post("/api/request_mail")
async def request_via_mail(mail_content: EmailSchema) -> JSONResponse:
    """request mail with some query"""
    mail_content.dict().update({"mail": ["Jayesh.Jadhav01@bajajallianz.co.in"]})
    # mail_content   
    _mail(mail_content)
...
if __name__ == '__main__':
    ...
    logging.info("Starting server on 1001L0721:5002")
    # uvicorn.run(app, host="0.0.0.0", port=5002)
    uvicorn.run(app, host="1001L0721", port=5002)
